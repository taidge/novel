...
pub fn preprocess_containers(input: &str) -> String {
    let open_re =
        Regex::new(r"^:::\s*(tip|warning|caution|danger|info|note|details|tabs|steps)(.*)$")
            .expect("valid regex");
    ...
    for line in input.lines() {
        if !in_container {
            if let Some(caps) = open_re.captures(line) {
                ...
                let ct = ContainerType::from_str(type_str).unwrap();
                let custom_title = caps.get(2).map(|m| m.as_str().trim()).unwrap_or("");
    ...
    if in_container {
        match &container_type {
            Some(ContainerType::Tabs) => {
                ...
                output.push_str("<div class=\"tabs-container\">\n<div class=\"tabs-header\">\n");
                for (i, header) in tab_headers.iter().enumerate() {
                    let active = if i == 0 { " active" } else { "" };
                    output.push_str(&format!(
                        "<button class=\"tab-btn{}\" data-tab=\"{}\">{}</button>\n",
