...
use anyhow::Result;
use sapid_shared::config::{SiteConfig, ThemeConfig};
use sapid_shared::{NavItem, PageData, PageType, SidebarItem};
use std::collections::HashMap;
use std::path::{Path, PathBuf};
use tracing::info;

use builder::{
    build_pages, copy_static_assets, generate_feed_xml, generate_sitemap_xml, route_to_file_path,
};
...
/// Builder for constructing and building an Sapid documentation site.
///
/// # Usage
///
/// **Standalone (with `sapid.toml`)**
/// ```no_run
...
impl Sapid {
    ...
    /// Load from a project root that contains `sapid.toml`.
    ...
    pub fn current_config(&self) -> &SiteConfig {
        &self.config
    }
    ...
    pub fn build(&self) -> Result<BuiltSite> {
        let br = build_pages(&self.project_root, &self.config)?;
        let engine = TemplateEngine::new(&self.project_root)?;
...
impl BuiltSite {
    ...
    pub fn write_to(&self, dir: impl AsRef<Path>) -> Result<()> {
        ...
        copy_static_assets(&docs_root, output_dir)?;
    ...
    pub fn write_to_default_output(&self) -> Result<()> {
        let dir = self.config.output_dir(&self.project_root);
        self.write_to(dir)
    }
}
